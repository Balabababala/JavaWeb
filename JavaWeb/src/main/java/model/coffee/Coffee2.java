...
public class Coffee2 {
	...
	//Coffee constructor
	public Coffee2(Double milk,Double coffee) {
		this.milk=milk;
		this.coffee=coffee;
	}
	public Coffee2(String milk,String coffee) {
	...
	public String getDesctiption() {
		return rules.stream()
			    .filter(rule-> rule.matches(milk, coffee))
			    .findFirst()
			    .map(CoffeeRule::getFinalDescription)
			    //.map(rule -> rule.getFinalDesctiption())
			    .orElse("無法辨識");
}
